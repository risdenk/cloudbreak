...
public class AwsNetworkConnector implements NetworkConnector {
    ...
    public CreatedCloudNetwork createNetworkWithSubnets(NetworkCreationRequest networkRequest) {
        ...
        AmazonCloudFormationRetryClient cloudFormationRetryClient = awsClient.createCloudFormationRetryClient(credentialView,
                networkRequest.getRegion().value());
        String cloudFormationTemplate = awsNetworkCfTemplateProvider.provide(networkRequest.getNetworkCidr(), networkRequest.getSubnetCidrs());
        ...
        Set<String> subnetIds = getCreatedSubnet(output, networkRequest.getSubnetCidrs().size());
